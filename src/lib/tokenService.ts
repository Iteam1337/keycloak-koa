...
export class TokenService {
  ...
  async extractUserFromToken(token: string, jwksService?: JwksService): Promise<User> {
    try {
  ...
  setCookies(res: any, { access_token, refresh_token, expires_in }: TokenData): void {
    // Handle both Express and Koa response objects
    if (res.cookie) {
      // Express
  ...
  clearAuthCookies(res: any): void {
    ...
    if (res.clearCookie) {
      ...
      // Koa
      res.cookies.set('auth_token', null, { maxAge: 0 });
      res.cookies.set('refresh_token', null, { maxAge: 0 });
    }
  }
}
